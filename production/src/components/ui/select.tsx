/**
 * Select — accessible dropdown (Radix-based, keyboard-friendly).
 *
 * @example
 * <Select value={state} onValueChange={setState}>
 *   <SelectTrigger placeholder="Pick a state" />
 *   <SelectContent>
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-[100000] max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-hairline bg-paper text-ink shadow-xl",
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("px-2 py-1.5 text-xs font-semibold text-ink-3 uppercase tracking-wider", className)}
    {...props}
  />
));
